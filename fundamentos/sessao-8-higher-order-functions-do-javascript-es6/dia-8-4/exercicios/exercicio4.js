...
todo De olho na dica 👀: Você pode acessar o index do array dentro de map, e você pode ver o objeto esperado na constante expected.
*/
...
function studentAverage() {
  ...
  const averageGrades = grades.map((element, index) => {
    /* console.log(`index[${index}] : valor (${element})`); */
    return `{name: ${students[index]}, 
    average: ${element.reduce((acc, curr)=>(acc + curr)   
    )/5} }`;
  });
